...
const memberSchema = z.object({
  ...
  citizenshipIssueDistrict: z.string().optional(),
  ...
  wardNo: z.coerce.number().optional(),
  tole: z.string().optional(),
  tempProvince: z.string().optional(),
  tempDistrict: z.string().optional(),
  tempMunicipality: z.string().optional(),
  tempWardNo: z.coerce.number().optional(),
  tempTole: z.string().optional(),
...
export default function MemberForm({ initialData }: MemberFormProps) {
  ...
  const [photos, setPhotos] = useState<{ [key: string]: string }>(
    initialData ? {
      ...
      citizenshipFrontUrl: initialData.citizenshipFrontUrl ?? "",
      citizenshipBackUrl: initialData.citizenshipBackUrl ?? "",
  ...
  // Address Sync State
  const [sameAsDocument, setSameAsDocument] = useState(
    initialData ? (initialData.tempProvince === initialData.province && initialData.tempProvince !== "") : false
  );
        ...
        <div className="flex justify-between items-center max-w-2xl mx-auto">
          {STEPS.map((s, idx) => (
            <div key={s.id} className="flex flex-col items-center gap-3 relative flex-1">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all z-10 ${
                step >= s.id ? "bg-brand-600 text-white shadow-lg shadow-brand-600/20" : "bg-white text-slate-400 border border-slate-200"
              }`}>
              ...
              {idx < STEPS.length - 1 && (
                <div className={`absolute top-6 -right-1/2 w-full h-[2px] ${
        ...
        {step === 3 && (
          <div className="space-y-8">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Primary Occupation</label>
                <input {...register("occupation")} className="w-full h-12 px-4 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:ring-4 ring-brand-500/5 outline-none transition-all" />
              </div>
              <div className="space-y-2">
